...
// origin:
    ...
    // http://userscripts.org/users/86496
    // 164: add fading animation to indicator bar, mod config box, skip word list
...
//{	values >
	...
	// var STYLE_COLOR   = [ '#719fa9', '#99ccff', '#ff99cc', '#66cc66', '#cc99ff', '#ffcc66', '#66aaaa', '#dd9966', '#aaaaaa', '#dd6699' ];
	var STYLE_COLOR   =  [ '#bbddbb44',  '#eed0b044']
	...
	var but_c = '#99cc99', but_ca = '#FFD000', but_cd = '#999999', but_cb = '#669966';
        // button normal/active/disable background color/border color.
...
// #### Config II #### --------------------------{{
    ...
    var skip_word_list = "the to in on among between and a an of by with".split(' ');
...
//			'#' + PRE + '_word_inputs_list > li > label > input[type=image]{vertical-align:top;padding:0;height:12px;}',
            ...
            '@keyframes fadeit {from {opacity:.6;} to {opacity:0;}}',
	...
	function move(node) {
		...
		if (node.getBoundingClientRect) {
			...
			if (Ewh_configs[5]) {
				...
				_em_bar.setAttribute( 'style', 'background:rgba(29,163,63,.3);position:absolute;width:100%;height:' + pos_h + 'px;top:' + pos_t + 'px; animation-name: fadeit; animation-duration: 3s;' );
			}
		} else {
			node.scrollIntoView();
		}
	...
	function init_words(word) {
		...
		if (erg) {
			...
			if (skip_word_list.length) {
				for (i in word_s) {
					for (var j in skip_word_list) {
                        if (word_s[i].toLowerCase() == skip_word_list[j]) words_off[i] = true;
                    }
				}
            }
		}//l(word_s[0].exp);
...
//			aside.style.width = Math.max(320,_aside_w) +'px';
	...
	function config_box() {
		...
		var confTitle = creaElemIn('h3', confBOX);
			confTitle.setAttribute('style', 'font-weight:800;border-bottom:1px solid black;width:100%;margin:15px auto 10px auto;');
